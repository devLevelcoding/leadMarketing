...
export async function getMoreLeads(limit: number, phase: number, excludeLeadIds: number[]) {
  const exc = excludeLeadIds.join(",");
  return fetch(`/api/warmup/more-leads?limit=${limit}&phase=${phase}&exclude=${exc}`).then(r => r.json());
}
