...
import { getTemplates, getWarmupPlan, getWarmupToday, getWarmupHistory, getWarmupUpcoming, getMoreLeads, getLighthouseScores, createWarmupPlan, deleteWarmupPlan, patchWarmupBatchLead, advanceWarmupDay, streamLighthouseScan, streamReportScan } from "./api";
...
export default function WarmupPage() {
  return <Suspense><WarmupPageInner /></Suspense>;
}
...
function PhaseWarmup({ phase, templates }: { phase: number; templates: Template[] }) {
  ...
  const [activeTab, setActiveTab]   = useState<"today" | "plan" | "plan90" | "history" | "next5" | "next10" | "countries" | "instagram" | "reddit">("today");
  ...
  async function startNextMonth() {
    if (!confirm("Start a new 30-day campaign for this phase from today?")) return;
    setStartingNext(true);
    await deleteWarmupPlan(phase);
    const today = new Date().toISOString().slice(0, 10);
    const res = await createWarmupPlan(phase, today);
    if (res.ok) {
      setStartDate(today);
      await fetchPlan();
      fetchToday();
      ...
      alert(err.error || "Failed to create new plan");
    }
    setStartingNext(false);
  }

  async function loadMoreLeads(limit: number) {
    if (!todayBatch) return;
    setLoadingMore(true);
    const existingIds = todayBatch.leads.map(bl => bl.lead.id);
    const data = await getMoreLeads(limit, phase, existingIds);
    if (data.leads?.length) {
      setTodayBatch(prev => prev
        ? { ...prev, leads: [...prev.leads, ...data.leads] }
        : prev
      );
    }
  ...
  async function setStatus(batchLeadId: number, status: string) {
    ...
    const refreshes: Promise<void>[] = [fetchPlan(true)];
  ...
  async function advanceDay() {
    setAdvancing(true);
    const res = await advanceWarmupDay(phase);
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      alert(err.error || "Failed to advance day");
      setAdvancing(false);
      return;
    }
  ...
  return (
    <div className="space-y-5">
        ...
        {([
          ...
          <button key={key} onClick={() => setActiveTab(key)}
            className={`px-5 py-2.5 text-sm font-medium whitespace-nowrap border-b-2 -mb-px transition ${
              activeTab === key
                ? key === "countries" ? "border-teal-600 text-teal-700" : key === "instagram" ? "border-pink-500 text-pink-600" : key === "reddit" ? "border-orange-500 text-orange-600" : "border-blue-600 text-blue-700"
      ...
      {activeTab === "today"     && <TodayTab batch={todayBatch} loading={todayLoading} updating={updating} onStatus={setStatus} templates={templates} showAdvanceModal={showAdvanceModal} advancing={advancing} onOpenModal={() => setShowAdvanceModal(true)} onAdvance={advanceDay} onCloseModal={() => setShowAdvanceModal(false)} page={todayPage} setPage={setTodayPage} lhScores={lhScores} lhScanning={lhScanning} lhProgress={lhProgress} onRunLhScan={runLhScan} repScanning={repScanning} repProgress={repProgress} onRunReportScan={runReportScan} expectedPhase={phase} isComplete={daysCompleted >= plan.totalDays} onStartNewMonth={startNextMonth} startingNext={startingNext} onLoadMore={loadMoreLeads} loadingMore={loadingMore} />}
