...
export function WaTodayTab({ batch, loading, updating, onStatus, showAdvanceModal, advancing, onOpenModal, onAdvance, onCloseModal, page, setPage, isComplete, onStartNewMonth, startingNext }: {
    ...
    <div className="bg-white border rounded-xl p-8 text-center text-gray-400">
      {isComplete ? (
        <>
          ...
          <p className="text-sm text-gray-400 mb-5">All 30 days sent. Ready to start Month 2?</p>
          {onStartNewMonth && (
            <button
              onClick={onStartNewMonth}
              disabled={startingNext}
              className="bg-green-600 text-white px-6 py-2.5 rounded-lg text-sm font-semibold hover:bg-green-700 disabled:opacity-50 transition"
            >
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex gap-2 text-sm flex-wrap items-center">
          ...
          <button
            onClick={() => { setWebsiteOnly(v => !v); setPage(1); }}
            className={`px-3 py-1 rounded-full text-sm font-medium border transition ${websiteOnly ? "bg-blue-600 text-white border-blue-600" : "bg-white text-gray-500 border-gray-200 hover:border-gray-400"}`}
          >
            🌐 {websiteOnly ? "Website only" : "All leads"}
          </button>
        </div>
        {allDone && (
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b">
            <tr>
              ...
              <th className="text-left px-4 py-3 font-medium text-gray-600 text-xs">Phone / WA</th>
