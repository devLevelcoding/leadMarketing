...
function FindWaButton({ lead, onFound }: { lead: Lead; onFound: (phone: string) => void }) {
  ...
  async function find() {
    setState("loading");
    try {
      const res = await fetch(`/api/find-phone?website=${encodeURIComponent(lead.website ?? lead.domain)}`);
      const data = await res.json();
      if (data.numbers?.length) { setNumbers(data.numbers); setState("found"); }
      else setState("none");
  ...
  if (state === "idle")
    return (
      <button onClick={find} className="text-xs bg-green-50 text-green-700 border border-green-200 px-2 py-1 rounded hover:bg-green-100 transition font-medium">
        🔍 Find WA
      </button>
    );
  if (state === "loading")
    return <span className="text-xs text-gray-400 animate-pulse">Searching…</span>;
  if (state === "none")
    return <span className="text-xs text-gray-400">No number found</span>;
  if (state === "error")
    return <span className="text-xs text-red-400">Error</span>;
  return (
    <div className="flex flex-wrap gap-1">
      {numbers.map(n => (
        <button key={n} onClick={() => savePhone(n)}
          className="text-xs bg-green-600 text-white px-2 py-0.5 rounded hover:bg-green-700 transition font-medium">
          {n} ✓ Save
        </button>
      ))}
...
export default function WaLeadRow({ index, bl, busy, onStatus }: {
  ...
  const lead = { ...bl.lead, phone: localPhone ?? bl.lead.phone };
  const rowBg =
    ...
    <tr className={`transition-colors ${rowBg}`}>
      ...
      <td className="px-4 py-3 text-xs">
        <div className="flex flex-col gap-1">
          ...
          {!lead.phone && (lead.website || lead.domain) && (
            <FindWaButton lead={lead} onFound={phone => setLocalPhone(phone)} />
          )}
        </div>
      </td>
      <td className="px-4 py-3 text-xs max-w-[160px]">
        {lead.website ? (
          <a href={lead.website.startsWith("http") ? lead.website : `https://${lead.website}`}
            ...
            className="text-blue-600 hover:underline truncate block" title={lead.website}>
            {lead.website.replace(/^https?:\/\/(www\.)?/, "")}
          </a>
        ) : lead.domain ? (
          <a href={`https://${lead.domain}`} target="_blank" rel="noopener noreferrer"
            className="text-blue-500 hover:underline truncate block opacity-60" title={lead.domain}>
            {lead.domain}
          </a>
        ) : <span className="text-gray-300">—</span>}
      </td>
